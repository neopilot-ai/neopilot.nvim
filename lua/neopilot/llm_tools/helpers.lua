...
---@return neopilot.ui.Confirm | nil
function M.confirm(message, callback, confirm_opts, session_ctx, tool_name)
  ...
  local Confirm = require("neopilot.ui.confirm")
  ...
  if not sidebar or not sidebar.containers.input or not sidebar.containers.input.winid then
    Utils.error("Neopilot sidebar not found", { title = "Neopilot" })
    callback(false)
    return
  end
...
function M.get_bufnr(abs_path)
  local sidebar = require("neopilot").get()
  if not sidebar then return 0, "Neopilot sidebar not found" end
