...
local NAMESPACE = vim.api.nvim_create_namespace("neopilot-diff")
local KEYBINDING_NAMESPACE = vim.api.nvim_create_namespace("neopilot-diff-keybinding")
...
function M.enabled()
  return require("neopilot.config").mode == "agentic" and not require("neopilot.config").behaviour.enable_fastapply
end
...
--- IMPORTANT: Using "the_diff" instead of "diff" is to avoid LLM streaming generating function parameters in alphabetical order, which would result in generating "path" after "diff", making it impossible to achieve a streaming diff view.
---@type NeopilotLLMToolFunc<{ path: string, the_diff?: string }>
...
For example:
  ...
  local augroup = vim.api.nvim_create_augroup("neopilot_replace_in_file", { clear = true })
