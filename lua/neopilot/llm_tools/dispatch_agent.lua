...
---@class neopilot.DispatchAgentInput
---@field prompt string

---@type neopilot.LLMToolOnRender<neopilot.DispatchAgentInput>
function M.on_render(input, opts)
  ...
  local hl = Highlights.NEOPILOT_TASK_RUNNING
  if result_message then
    ...
    if result then
      if result.is_error then
        state = "failed"
        icon = Utils.icon("❌ ")
        hl = Highlights.NEOPILOT_TASK_FAILED
      else
        state = "completed"
        icon = Utils.icon("✅ ")
        hl = Highlights.NEOPILOT_TASK_COMPLETED
      end
    end
  end
  local lines = {}
...
---@type NeopilotLLMToolFunc<neopilot.DispatchAgentInput>
...
When you're done, provide a clear and concise summary of what you found.]]):gsub("${prompt}", prompt)
  ...
  ---@type neopilot.AgentLoopOptions
