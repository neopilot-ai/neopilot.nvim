...
---@type NeopilotLLMToolFunc<{ path: string }>
function M.read_file_toplevel_symbols(input, opts)
  ...
  if not repo_map_lib then return "", "Failed to load neopilot_repo_map" end
...
---@type NeopilotLLMToolFunc<{ command: "view" | "str_replace" | "create" | "insert" | "undo_edit", path: string, old_str?: string, new_str?: string, file_text?: string, insert_line?: integer, new_str?: string, view_range?: integer[] }>
function M.str_replace_editor(input, opts)
  if input.command == "undo_edit" then return require("neopilot.llm_tools.undo_edit").func(input, opts) end
  ---@cast input any
  return M.str_replace_based_edit_tool(input, opts)
end

---@type NeopilotLLMToolFunc<{ command: "view" | "str_replace" | "create" | "insert", path: string, old_str?: string, new_str?: string, file_text?: string, insert_line?: integer, new_str?: string, view_range?: integer[] }>
function M.str_replace_based_edit_tool(input, opts)
  ...
  if input.command == "view" then
    local view = require("neopilot.llm_tools.view")
  ...
  if input.command == "str_replace" then
    ...
    return require("neopilot.llm_tools.str_replace").func(input, opts)
  end
  if input.command == "create" then return require("neopilot.llm_tools.create").func(input, opts) end
  if input.command == "insert" then return require("neopilot.llm_tools.insert").func(input, opts) end
  return false, "Unknown command: " .. input.command
end

---@type NeopilotLLMToolFunc<{ abs_path: string }>
...
---@type NeopilotLLMToolFunc<{ abs_path: string, content: string }>
...
---@type NeopilotLLMToolFunc<{ source_path: string, destination_path: string }>
...
---@type NeopilotLLMToolFunc<{ query: string }>
...
---@type NeopilotLLMToolFunc<{ message: string, scope?: string }>
...
---@type NeopilotLLMToolFunc<{ code: string, path: string, container_image?: string }>
...
---@param history_messages NeopilotLLMMessage[]
---@return NeopilotLLMTool[]
...
---@type NeopilotLLMTool[]
M._tools = {
  require("neopilot.llm_tools.replace_in_file"),
  require("neopilot.llm_tools.dispatch_agent"),
  require("neopilot.llm_tools.glob"),
  {
    name = "rag_search",
  ...
  require("neopilot.llm_tools.ls"),
  require("neopilot.llm_tools.grep"),
  require("neopilot.llm_tools.delete_tool_use_messages"),
  require("neopilot.llm_tools.add_todos"),
  require("neopilot.llm_tools.update_todo_status"),
  {
...
Then you can use the "read_definitions" tool to retrieve the source code definitions of these symbols.
  ...
  require("neopilot.llm_tools.str_replace"),
  require("neopilot.llm_tools.view"),
  require("neopilot.llm_tools.write_to_file"),
  require("neopilot.llm_tools.insert"),
  require("neopilot.llm_tools.undo_edit"),
  {
...
You can delete the first file by providing a path of "directory1/a/something.txt"
  ...
  require("neopilot.llm_tools.think"),
  require("neopilot.llm_tools.get_diagnostics"),
  require("neopilot.llm_tools.bash"),
  require("neopilot.llm_tools.attempt_completion"),
  require("neopilot.llm_tools.edit_file"),
  {
    name = "web_search",
...
---@class neopilot.ProcessToolUseOpts
---@field session_ctx table
---@field on_log? fun(tool_id: string, tool_name: string, log: string, state: NeopilotLLMToolUseState): nil
...
---@param tools NeopilotLLMTool[]
---@param tool_use NeopilotLLMToolUse
...
function M.process_tool_use(tools, tool_use, opts)
  ...
  elseif tool_use.name == "str_replace_based_edit_tool" then
    ...
    ---@type NeopilotLLMTool?
    local tool = vim.iter(tools):find(function(tool) return tool.name == tool_use.name end) ---@param tool NeopilotLLMTool
