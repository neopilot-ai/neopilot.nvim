...
local Message = require("neopilot.history.message")
...
---@field use NeopilotLLMToolUse
---@field result? NeopilotLLMToolResult
---@field result_message? neopilot.HistoryMessage Complete result message
...
local function update_view_result(tool_info, stale_view)
  ...
  if stale_view then
    ...
    local view_result, view_error = require("neopilot.llm_tools.view").func(
...
local function generate_view_messages(tool_use, path, stale_view)
  ...
  if stale_view then
    ...
    view_result, view_error = require("neopilot.llm_tools.view").func({ path = path }, {})
  end
...
---@param messages neopilot.HistoryMessage[]
---@return NeopilotPartialLLMToolUse[]
---@return neopilot.HistoryMessage[]
function M.get_pending_tools(messages)
  local last_turn_id = nil
  if #messages > 0 then last_turn_id = messages[#messages].turn_id end

  local pending_tool_uses = {} ---@type NeopilotPartialLLMToolUse[]
  local pending_tool_uses_messages = {} ---@type neopilot.HistoryMessage[]
