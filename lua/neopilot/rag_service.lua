...
local container_name = "neopilot-rag-service"
local service_path = "/tmp/" .. container_name

function M.get_rag_service_image() return "ghcr.io/neopilot-ai/neopilot-rag-service:0.0.11" end
...
function M.get_data_path()
  local p = Path:new(vim.fn.stdpath("data")):joinpath("neopilot/rag_service")
...
function M.launch_rag_service(cb)
  ...
  then
    ...
    if llm_api_key == nil or llm_api_key == "" then
      error(string.format("cannot launch neopilot rag service, %s is not set", Config.rag_service.llm.api_key))
      return
    end
  end
  ...
  then
    ...
    if embed_api_key == nil or embed_api_key == "" then
      error(string.format("cannot launch neopilot rag service, %s is not set", Config.rag_service.embed.api_key))
      return
    end
  end
  ...
  elseif M.get_rag_service_runner() == "nix" then
    ...
    local dirname =
      Utils.trim(string.sub(debug.getinfo(1).source, 2, #"/lua/neopilot/rag_service.lua" * -1), { suffix = "/" })
...
---@class NeopilotRagServiceAddResourceResponse
---@field status string
...
---@class NeopilotRagServiceRetrieveSource
---@field uri string
---@field content string

---@class NeopilotRagServiceRetrieveResponse
---@field response string
---@field sources NeopilotRagServiceRetrieveSource[]

---@param base_uri string
---@param query string
---@param on_complete fun(resp: NeopilotRagServiceRetrieveResponse | nil, error: string | nil): nil
...
---@class NeopilotRagServiceIndexingStatusSummary
---@field indexing integer
---@field completed integer
---@field failed integer

---@class NeopilotRagServiceIndexingStatusResponse
---@field uri string
---@field is_watched boolean
---@field total_files integer
---@field status_summary NeopilotRagServiceIndexingStatusSummary

---@param uri string
---@return NeopilotRagServiceIndexingStatusResponse | nil
...
---@class NeopilotRagServiceResource
---@field name string
---@field uri string
---@field type string
---@field status string
...
---@class NeopilotRagServiceResourceListResponse
---@field resources NeopilotRagServiceResource[]
---@field total_count number

---@return NeopilotRagServiceResourceListResponse | nil
