...
local JsonParser = require("neopilot.libs.jsonparser")
...
---@param tool NeopilotLLMTool
---@return NeopilotClaudeTool
...
---@return NeopilotClaudeMessage[]
function M:parse_messages(opts)
  ---@type NeopilotClaudeMessage[]
  local messages = {}
  ...
  for _, message in ipairs(opts.messages) do
    ...
    elseif type(content_items) == "table" then
      ---@cast content_items NeopilotLLMMessageContentItem[]
...
---@param usage neopilot.AnthropicTokenUsage | nil
---@return neopilot.LLMTokenUsage | nil
function M.transform_anthropic_usage(usage)
  if not usage then return nil end
  ---@type neopilot.LLMTokenUsage
  local res = {
...
function M:parse_response(ctx, data_stream, event_state, opts)
  ...
  ---@param content NeopilotLLMMessageContentItem
  ---@param uuid? string
  ...
  if self.support_prompt_caching then
    if #messages > 0 then
      ...
      for i = #messages, 1, -1 do
        ...
        ---@cast message NeopilotClaudeMessage
        local content = message.content
        ---@cast content NeopilotClaudeMessageContentTextItem[]
...
function M.on_error(result)
  ...
  if not result.body then
    return Utils.error("API request failed with status " .. result.status, { once = true, title = "Neopilot" })
  end

  local ok, body = pcall(vim.json.decode, result.body)
  if not (ok and body and body.error) then
    return Utils.error("Failed to parse error response", { once = true, title = "Neopilot" })
  end
  ...
  Utils.error(error_msg, { once = true, title = "Neopilot" })
end
