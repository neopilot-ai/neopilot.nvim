...
---@class neopilot.Providers
---@field openai NeopilotProviderFunctor
---@field claude NeopilotProviderFunctor
---@field copilot NeopilotProviderFunctor
---@field azure NeopilotProviderFunctor
---@field gemini NeopilotProviderFunctor
---@field cohere NeopilotProviderFunctor
---@field bedrock NeopilotBedrockProviderFunctor
---@field ollama NeopilotProviderFunctor
---@field vertex_claude NeopilotProviderFunctor
local M = {}
...
---@param Opts NeopilotSupportedProvider | NeopilotProviderFunctor | NeopilotBedrockProviderFunctor
---@return string | nil
function E.parse_envvar(Opts)
  -- First try the scoped version (e.g., NEOPILOT_ANTHROPIC_API_KEY)
  local scoped_key_name = nil
  if Opts.api_key_name and type(Opts.api_key_name) == "string" and Opts.api_key_name ~= "" then
    -- Only add NEOPILOT_ prefix if it's a regular environment variable (not a cmd: or already prefixed)
    if not Opts.api_key_name:match("^cmd:") and not Opts.api_key_name:match("^NEOPILOT_") then
      scoped_key_name = "NEOPILOT_" .. Opts.api_key_name
    end
  end
...
---@param opts {refresh: boolean, provider: NeopilotProviderFunctor | NeopilotBedrockProviderFunctor}
---@private
  ...
  local function on_confirm(value)
    if value then
      ...
      if not opts.provider.is_env_set() then
        Utils.warn("Failed to set " .. var .. ". Neopilot won't work as expected", { once = true })
      end
    end
  end
  ...
  local function mount_input_ui()
    vim.defer_fn(function()
      ...
      if not vim.tbl_contains(exclude_filetypes, vim.bo.filetype) and not opts.provider.is_env_set() then
        local Input = require("neopilot.ui.input")
...
E.REQUEST_LOGIN_PATTERN = "NeopilotRequestLogin"

---@param provider NeopilotDefaultBaseProvider
function E.require_api_key(provider) return provider.api_key_name ~= nil and provider.api_key_name ~= "" end

M.env = E

M = setmetatable(M, {
  ---@param t neopilot.Providers
  ---@param k neopilot.ProviderName
  __index = function(t, k)
    ...
    if provider_config.__inherited_from ~= nil then
      ...
      local ok, module = pcall(require, "neopilot.providers." .. provider_config.__inherited_from)
      ...
      local ok, module = pcall(require, "neopilot.providers." .. k)
      if ok then
      ...
      elseif provider_config.parse_curl_args == nil then
        error(
          string.format(
            'The configuration of your provider "%s" is incorrect, missing the `__inherited_from` attribute or a custom `parse_curl_args` function. Please fix your provider configuration. For more details, see: https://github.com/neopilot-ai/neopilot.nvim/wiki/Custom-providers',
            k
          )
        )
      end
    ...
    if rawget(t[k], "setup") == nil then
      ...
      t[k].setup = function()
        ...
        require("neopilot.tokenizers").setup(t[k].tokenizer_id)
      end
    end
...
function M.setup()
  vim.g.neopilot_login = false

  ---@type NeopilotProviderFunctor | NeopilotBedrockProviderFunctor
...
function M.refresh(provider_name)
  require("neopilot.config").override({ provider = provider_name })

  ---@type NeopilotProviderFunctor | NeopilotBedrockProviderFunctor
  local p = M[Config.provider]
  E.setup({ provider = p, refresh = true })
  Utils.info("Switch to provider: " .. provider_name, { once = true, title = "Neopilot" })
end

---@param opts NeopilotProvider | NeopilotSupportedProvider | NeopilotProviderFunctor | NeopilotBedrockProviderFunctor
---@return NeopilotDefaultBaseProvider provider_opts
---@return table<string, any> request_body
function M.parse_config(opts)
  ---@type NeopilotDefaultBaseProvider
  local provider_opts = {}
