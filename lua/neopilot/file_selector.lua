...
local Selector = require("neopilot.ui.selector")

local PROMPT_TITLE = "(Neopilot) Add a file"
...
function FileSelector:get_filepaths()
  if type(Config.file_selector.provider_opts.get_filepaths) == "function" then
    ---@type neopilot.file_selector.opts.IGetFilepathsParams
    local params = {
  ...
  vim.schedule(function()
    if Config.file_selector.provider ~= nil then
      ...
      if type(Config.file_selector.provider) == "function" then
        ...
        local params = { title = title, filepaths = filepaths, handler = handler } ---@type neopilot.file_selector.IParams
        ...
        ---@cast provider neopilot.SelectorProvider
