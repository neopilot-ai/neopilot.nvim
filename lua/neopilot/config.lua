...
---@alias neopilot.InputProvider "native" | "dressing" | "snacks" | fun(input: neopilot.ui.Input): nil

local Utils = require("neopilot.utils")

---@class neopilot.file_selector.IParams
---@field public title      string
---@field public filepaths  string[]
---@field public handler    fun(filepaths: string[]|nil): nil

---@class neopilot.file_selector.opts.IGetFilepathsParams
---@field public cwd                string
---@field public selected_filepaths string[]

---@class neopilot.CoreConfig: neopilot.Config
local M = {}
---@class neopilot.Config
M._defaults = {
  debug = false,
  ---@alias neopilot.Mode "agentic" | "legacy"
  ---@type neopilot.Mode
  mode = "agentic",
  ---@alias neopilot.ProviderName "claude" | "openai" | "azure" | "gemini" | "vertex" | "cohere" | "copilot" | "bedrock" | "ollama" | string
  ---@type neopilot.ProviderName
  provider = "claude",
  -- WARNING: Since auto-suggestions are a high-frequency operation and therefore expensive,
  -- currently designating it as `copilot` provider is dangerous because: https://github.com/neopilot-ai/neopilot.nvim/issues/1048
  ...
  ---See https://github.com/neopilot-ai/neopilot.nvim/wiki#custom-providers for more details
  ---@type {[string]: NeopilotProvider}
  providers = {
    ---@type NeopilotSupportedProvider
    openai = {
  ...
  ---How it works:
  --- When dual_boost is enabled, neopilot will generate two responses from the first_provider and second_provider respectively. Then use the response from the first_provider as provider1_output and the response from the second_provider as provider2_output. Finally, neopilot will generate a response based on the prompt and the two reference outputs, with the default Provider as normal.
  ...
  ---Specify the behaviour of neopilot.nvim
  ---1. auto_focus_sidebar              : Whether to automatically focus the sidebar when opening neopilot.nvim. Default to true.
  ...
  ---                                     Note that neopilot will safely set these keymap. See https://github.com/neopilot-ai/neopilot.nvim/wiki#keymaps-and-api-i-guess for more details.
  ...
  ---@class NeopilotConfigSuggestion
  ---@field enabled boolean
  ---@field min_chars integer
  ---@field accept_key string
  ---@field accept_word_key string|nil
  ---@field accept_line_key string|nil
  ---@field next_suggestion_key string|nil
  ---@field prev_suggestion_key string|nil
  ---@field show_suggestion_on_enter boolean
  ---@field debounce integer
  ---@field max_context_lines integer Maximum number of lines to include in context
  ---@field chunk_size integer Size of chunks for processing large files
  ---@field cache_ttl integer Time-to-live for cache entries in seconds
  suggestion = {
    enabled = true,
    min_chars = 1,
    debounce = 100,
  ...
  prompt_logger = { -- logs prompts to disk (timestamped, for replay/debugging)
    ...
    log_dir = Utils.join_paths(vim.fn.stdpath("cache"), "neopilot_prompts"), -- directory where logs are saved
  ...
  history = {
    ...
    storage_path = Utils.join_paths(vim.fn.stdpath("state"), "neopilot"),
    paste = {
      extension = "png",
  ...
  mappings = {
    ---@class NeopilotConflictMappings
    diff = {
      ours = "co",
      theirs = "ct",
      all_theirs = "ca",
    ...
    -- NOTE: The following will be safely set by neopilot.nvim
    ask = "<leader>aa",
    new_ask = "<leader>an",
    edit = "<leader>ae",
    refresh = "<leader>ar",
    ...
    sidebar = {
      ...
      add_file = "@",
      close = { "q" },
      ---@alias NeopilotCloseFromInput { normal: string | nil, insert: string | nil }
      ---@type NeopilotCloseFromInput | nil
  ...
  windows = {
    ---@alias NeopilotPosition "right" | "left" | "top" | "bottom" | "smart"
    ---@type NeopilotPosition
    position = "right",
    fillchars = "eob: ",
    ...
    ask = {
      floating = false, -- Open the 'NeopilotAsk' prompt in a floating window
      border = { " ", " ", " ", " ", " ", " ", " ", " " },
      start_insert = true, -- Start insert mode when opening the ask window
      ---@alias NeopilotInitialDiff "ours" | "theirs"
      ---@type NeopilotInitialDiff
      focus_on_apply = "ours", -- which diff to focus after applying
    },
  },
  --- @class NeopilotConflictConfig
  diff = {
    autojump = true,
  ...
  --- @class NeopilotHintsConfig
  hints = {
    enabled = true,
  },
  --- @class NeopilotRepoMapConfig
  repo_map = {
    ignore_patterns = { "%.git", "%.worktree", "__pycache__", "node_modules" }, -- ignore files matching these
    negate_patterns = {}, -- negate ignore files matching these.
  },
  --- @class NeopilotFileSelectorConfig
  file_selector = {
    provider = nil,
  ...
  selector = {
    ---@alias neopilot.SelectorProvider "native" | "fzf_lua" | "mini_pick" | "snacks" | "telescope" | fun(selector: neopilot.ui.Selector): nil
    ---@type neopilot.SelectorProvider
    provider = "native",
    provider_opts = {},
  ...
  ---@type NeopilotLLMToolPublic[] | fun(): NeopilotLLMToolPublic[]
  custom_tools = {},
  ---@type NeopilotSlashCommand[]
  slash_commands = {},
  ---@type NeopilotShortcut[]
  shortcuts = {},
}
...
function M.save_last_model(model_name, provider_name)
  local config_dir = Utils.join_paths(vim.fn.expand("~"), ".config", "neopilot.nvim")
  ...
  if file then
    ...
    if content and content ~= "" then
      ...
      elseif success and data and data.last_model then
        ...
        Utils.warn("Invalid or corrupt JSON in last model file: " .. storage_path, { title = "Neopilot" })
      end
    else
      Utils.warn("Last model file is empty: " .. storage_path, { title = "Neopilot" })
    end
  end
  return nil
end
...
function M.setup(opts)
  ...
  local migration_url = "https://github.com/neopilot-ai/neopilot.nvim/wiki/Provider-configuration-migration-guide"
...
---@param provider_name neopilot.ProviderName
function M.get_provider_config(provider_name)
  local found = false
  local config = {}

  if M.providers[provider_name] then
    found = true
  ...
  if type(config.suggestion.max_context_lines) ~= "number" or config.suggestion.max_context_lines < 100 then
    config.suggestion.max_context_lines = 1000
    vim.notify("Invalid max_context_lines, using default: 1000", vim.log.levels.WARN)
  end

  if type(config.suggestion.chunk_size) ~= "number" or config.suggestion.chunk_size < 10 then
    config.suggestion.chunk_size = 200
    vim.notify("Invalid chunk_size, using default: 200", vim.log.levels.WARN)
  end

  if type(config.suggestion.cache_ttl) ~= "number" or config.suggestion.cache_ttl < 0 then
    config.suggestion.cache_ttl = 300
    vim.notify("Invalid cache_ttl, using default: 300s", vim.log.levels.WARN)
  end
