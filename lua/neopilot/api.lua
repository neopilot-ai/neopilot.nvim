...
local PromptInput = require("neopilot.ui.prompt_input")

---@class neopilot.ApiToggle
---@operator call(): boolean
---@field debug ToggleBind.wrap
---@field hint ToggleBind.wrap

---@class neopilot.Api
---@field toggle neopilot.ApiToggle
local M = {}

---@param target_provider neopilot.SelectorProvider
function M.switch_selector_provider(target_provider)
  require("neopilot.config").override({
    selector = {
...
---@param target_provider neopilot.InputProvider
...
---@param target neopilot.ProviderName
function M.switch_provider(target) require("neopilot.providers").refresh(target) end
...
function M.build(opts)
  ...
  if opts.source and not vim.fn.executable("cargo") then
    error("Building neopilot.nvim requires cargo to be installed.", 2)
  end
...
function M.ask(opts)
  ...
  if Utils.is_sidebar_buffer(0) and not has_question and not new_chat then
    require("neopilot").close_sidebar()
    return false
  end
  ...
  local function ask(input)
    ...
    require("neopilot").open_sidebar(opts)
    ...
    vim.api.nvim_exec_autocmds("User", { pattern = "NeopilotInputSubmitted", data = { request = input } })
    return true
  end
...
function M.edit(request, line1, line2)
  local _, selection = require("neopilot").get()
  if not selection then return end
  selection:create_editing_input(request, line1, line2)
  if request ~= nil and request ~= "" then
    vim.api.nvim_exec_autocmds("User", { pattern = "NeopilotEditSubmitted", data = { request = request } })
  end
end

---@return neopilot.Suggestion | nil
function M.get_suggestion()
  local _, _, suggestion = require("neopilot").get()
  return suggestion
end
...
function M.select_model() require("neopilot.model_selector").open() end

function M.select_history()
  local buf = vim.api.nvim_get_current_buf()
  require("neopilot.history_selector").open(buf, function(filename)
    vim.api.nvim_buf_call(buf, function()
      if not require("neopilot").is_sidebar_open() then require("neopilot").open_sidebar({}) end
...
function M.stop() require("neopilot.llm").cancel_inflight_request() end

return setmetatable(M, {
  __index = function(t, k)
    local module = require("neopilot")
    ---@class AvailableApi: ApiCaller
    ---@field api? boolean
    local has = module[k]
    if type(has) ~= "table" or not has.api then
      Utils.warn(k .. " is not a valid neopilot's API method", { once = true })
      return
    end
    t[k] = has
    return t[k]
