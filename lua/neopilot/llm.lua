...
M.CANCEL_PATTERN = "NeopilotLLMEscape"

------------------------------Prompt and type------------------------------

local group = api.nvim_create_augroup("neopilot_llm", { clear = true })

---@param prev_memory string | nil
---@param history_messages neopilot.HistoryMessage[]
---@param cb fun(memory: neopilot.ChatMemory | nil): nil
...
---@class neopilot.AgentLoopOptions
...
---@param opts neopilot.AgentLoopOptions
...
---@param opts NeopilotGeneratePromptsOptions
---@return NeopilotPromptOptions
function M.generate_prompts(opts)
  ...
  if opts.project_context ~= nil and opts.project_context ~= "" and opts.project_context ~= "null" then
    local project_context = Path.prompts.render_file("_project.neopilotrules", template_opts)
  ...
  if opts.diagnostics ~= nil and opts.diagnostics ~= "" and opts.diagnostics ~= "null" then
    local diagnostics = Path.prompts.render_file("_diagnostics.neopilotrules", template_opts)
    if diagnostics ~= "" then
  ...
  if #selected_files > 0 or opts.selected_code ~= nil then
    local code_context = Path.prompts.render_file("_context.neopilotrules", template_opts)
  ...
  if opts.memory ~= nil and opts.memory ~= "" and opts.memory ~= "null" then
    local memory = Path.prompts.render_file("_memory.neopilotrules", template_opts)
    if memory ~= "" then
...
function M.curl(opts)
  ...
  local stopped = false
  ---@param stop_opts NeopilotLLMStopCallbackOptions
    ...
    callback = function(result)
      ...
      if result.status >= 400 then
        if provider.on_error then
          ...
          Utils.error("API request failed with status " .. result.status, { once = true, title = "Neopilot" })
        end
...
---@param opts NeopilotLLMStreamOptions
function M._stream(opts)
  ...
  ---@cast provider NeopilotProviderFunctor
  ...
  local handler_opts = {
    ...
    on_stop = function(stop_opts)
      ...
      ---@param tool_uses NeopilotPartialLLMToolUse[]
      ---@param tool_use_index integer
      ---@param tool_results NeopilotLLMToolResult[]
      ...
      if stop_opts.reason == "rate_limit" then
        ...
        if timer then
          ...
          Utils.info("Rate limit reached. Retrying in " .. retry_count .. " seconds", { title = "Neopilot" })
