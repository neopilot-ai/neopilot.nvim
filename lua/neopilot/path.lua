...
function History.list(bufnr)
  ...
  table.sort(res, function(a, b)
    local H = require("neopilot.history")
...
---@param mode NeopilotLlmMode
---@return string
function Prompt.get_custom_prompts_filepath(mode) return string.format("custom.%s.neopilotrules", mode) end

function Prompt.get_builtin_prompts_filepath(mode) return string.format("%s.neopilotrules", mode) end

---@class NeopilotTemplates
---@field initialize fun(cache_directory: string, project_directory: string): nil
---@field render fun(template: string, context: NeopilotTemplateOptions): string
...
function Prompt.get_templates_dir(project_root)
  if not P.available() then error("Make sure to build neopilot (missing neopilot_templates)", 2) end
  ...
  local function find_rules(dir)
    ...
    for _, entry in ipairs(scanner) do
      ...
      if file:is_file() then
        ...
        local mode = piece:match("([^.]+)%.neopilotrules$")
  ...
  local source_dir =
    Path:new(debug.getinfo(1).source:match("@?(.*/)"):gsub("/lua/neopilot/path.lua$", "") .. "templates")
      ...
      if user_template_path:exists() then
        ...
        for _, entry in ipairs(user_scanner) do
          ...
          if file:is_file() then
            ...
            if piece == "base.neopilotrules" then
...
---@param path string
---@param opts NeopilotTemplateOptions
...
---@return NeopilotTemplates|nil
function P._init_templates_lib()
  if _templates_lib ~= nil then return _templates_lib end
  local ok, module = pcall(require, "neopilot_templates")
  ---@cast module NeopilotTemplates
  ---@cast ok boolean
...
function P.setup()
  ...
  local cache_path = Path:new(Utils.join_paths(vim.fn.stdpath("cache"), "neopilot"))
  if not cache_path:exists() then cache_path:mkdir({ parents = true }) end
  P.cache_path = cache_path

  local data_path = Path:new(Utils.join_paths(vim.fn.stdpath("data"), "neopilot"))
