...
--- @class NeopilotConflictHighlights
...
local NAMESPACE = api.nvim_create_namespace("neopilot-conflict")
local KEYBINDING_NAMESPACE = api.nvim_create_namespace("neopilot-conflict-keybinding")
local AUGROUP_NAME = "neopilot_conflicts"
...
local function parse_buffer(bufnr, range_start, range_end)
  ...
  if prev_conflicts ~= has_conflict or not vim.b[bufnr].neopilot_conflict_mappings_set then
    local pattern = has_conflict and "NeopilotConflictDetected" or "NeopilotConflictResolved"
...
function H.setup_buffer_mappings(bufnr)
  ---@param desc string
  local function opts(desc) return { silent = true, buffer = bufnr, desc = "neopilot(conflict): " .. desc } end
  ...
  vim.b[bufnr].neopilot_conflict_mappings_set = true
end

---@param bufnr integer
function H.clear_buffer_mappings(bufnr)
  if not bufnr or not vim.b[bufnr].neopilot_conflict_mappings_set then return end

  for _, diff_mapping in pairs(Config.mappings.diff) do
    pcall(vim.api.nvim_buf_del_keymap, bufnr, "n", diff_mapping)
  end

  vim.b[bufnr].neopilot_conflict_mappings_set = false
  M.restore_timeoutlen(bufnr)
end

---@param bufnr integer
function M.override_timeoutlen(bufnr)
  if vim.b[bufnr].neopilot_original_timeoutlen then return end
  if Config.diff.override_timeoutlen > 0 then
    vim.b[bufnr].neopilot_original_timeoutlen = vim.o.timeoutlen
...
function M.restore_timeoutlen(bufnr)
  if vim.b[bufnr].neopilot_original_timeoutlen then
    vim.o.timeoutlen = vim.b[bufnr].neopilot_original_timeoutlen
    vim.b[bufnr].neopilot_original_timeoutlen = nil
  end
end
...
function M.setup()
  ...
  api.nvim_create_autocmd("User", {
    group = M.augroup,
    pattern = "NeopilotConflictDetected",
  ...
  api.nvim_create_autocmd("User", {
    group = M.augroup,
    pattern = "NeopilotConflictResolved",
    callback = function(ev)
