...
---@class NeopilotRoot
---@field paths string[]
---@field spec NeopilotRootSpec

---@alias NeopilotRootFn fun(buf: number): (string|string[])

---@alias NeopilotRootSpec string|string[]|NeopilotRootFn

---@type NeopilotRootSpec[]
...
function M.bufpath(buf)
  if buf == nil or type(buf) ~= "number" then
    ...
    vim.notify("neopilot: M.bufpath received invalid buffer: " .. tostring(buf), vim.log.levels.WARN)
    return nil
  end
  ...
  if not success then
    ...
    vim.notify(
      "neopilot: nvim_buf_get_name failed for buffer " .. tostring(buf) .. ": " .. tostring(result),
      vim.log.levels.WARN
    )
    return nil
  end
...
---@param spec NeopilotRootSpec
---@return NeopilotRootFn
...
---@param opts? { buf?: number, spec?: NeopilotRootSpec[], all?: boolean }
function M.detect(opts)
  opts = opts or {}
  opts.spec = opts.spec or type(vim.g.root_spec) == "table" and vim.g.root_spec or M.spec
  opts.buf = (opts.buf == nil or opts.buf == 0) and vim.api.nvim_get_current_buf() or opts.buf

  local ret = {} ---@type NeopilotRoot[]
