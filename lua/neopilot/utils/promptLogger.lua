...
---@class neopilot.utils.promptLogger
local M = {}

function M.log_prompt(request)
  local log_dir = Config.prompt_logger.log_dir
  local log_file = Utils.join_paths(log_dir, "neopilot_prompt_" .. os.date("%Y%m%d_%H%M%S") .. ".log")
...
local function refresh_logs()
  ...
  logs = vim.fn.glob(Utils.join_paths(dir, "neopilot_prompt_*.log"), false, true)
...
local function load_log(step)
  ...
  vim.b[buf].neopilot_logpath = logs[idx]
end
