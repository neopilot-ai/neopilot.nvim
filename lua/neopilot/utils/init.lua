...
local diff2search_replace = require("neopilot.utils.diff2search_replace")

---@class neopilot.utils: LazyUtilCore
---@field tokens neopilot.utils.tokens
---@field root neopilot.utils.root
---@field file neopilot.utils.file
---@field path neopilot.utils.path
---@field environment neopilot.utils.environment
---@field lsp neopilot.utils.lsp
---@field logger neopilot.utils.promptLogger
local M = {}
...
function M.safe_keymap_set(mode, lhs, rhs, opts)
  ...
  if not ok then
    M.debug("lazy.nvim is not available. Neopilot will use vim.keymap.set")
...
---@return neopilot.SelectionResult | nil Selected content and range
function M.get_visual_selection_and_range()
  if not M.in_visual_mode() then return nil end

  local Range = require("neopilot.range")
  local SelectionResult = require("neopilot.selection_result")
...
function M.debug(...)
  if not require("neopilot.config").debug then return end
  ...
  local formated_args = {
    "[" .. timestamp .. "] [NEOPILOT] [DEBUG] [" .. caller_module .. ":" .. info.currentline .. "]",
  }
...
local sidebar_buffer_var_name = "is_neopilot_sidebar_buffer"
...
---@param old_lines neopilot.ui.Line[]
---@param new_lines neopilot.ui.Line[]
---@return { start_line: integer, end_line: integer, content: neopilot.ui.Line[] }[]
...
---Dev icons are an optional install for neopilot, this function prevents ugly chars
...
---@param fields NeopilotLLMToolParamField[]
...
function M.get_commands()
  ...
  ---@type {[NeopilotSlashCommandBuiltInName]: NeopilotSlashCommandCallback}
  local builtin_cbs = {
  ...
  local builtin_commands = vim
    .iter(builtin_items)
    :map(
      ---@param item NeopilotSlashCommand
      function(item)
        return {
          name = item.name,
