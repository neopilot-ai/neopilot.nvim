...
local Path = require("neopilot.path")
local Config = require("neopilot.config")
local Diff = require("neopilot.diff")
local Llm = require("neopilot.llm")
local Utils = require("neopilot.utils")
local PromptLogger = require("neopilot.utils.promptLogger")
local Highlights = require("neopilot.highlights")
local RepoMap = require("neopilot.repo_map")
local FileSelector = require("neopilot.file_selector")
local LLMTools = require("neopilot.llm_tools")
local History = require("neopilot.history")
local Render = require("neopilot.history.render")
local Line = require("neopilot.ui.line")
local LRUCache = require("neopilot.utils.lru_cache")

local RESULT_BUF_NAME = "NEOPILOT_RESULT"
local VIEW_BUFFER_UPDATED_PATTERN = "NeopilotViewBufferUpdated"
local CODEBLOCK_KEYBINDING_NAMESPACE = api.nvim_create_namespace("NEOPILOT_CODEBLOCK_KEYBINDING")
local USER_REQUEST_BLOCK_KEYBINDING_NAMESPACE = api.nvim_create_namespace("NEOPILOT_USER_REQUEST_BLOCK_KEYBINDING")
local SELECTED_FILES_HINT_NAMESPACE = api.nvim_create_namespace("NEOPILOT_SELECTED_FILES_HINT")
local SELECTED_FILES_ICON_NAMESPACE = api.nvim_create_namespace("NEOPILOT_SELECTED_FILES_ICON")
local INPUT_HINT_NAMESPACE = api.nvim_create_namespace("NEOPILOT_INPUT_HINT")
local STATE_NAMESPACE = api.nvim_create_namespace("NEOPILOT_STATE")
local RESULT_BUF_HL_NAMESPACE = api.nvim_create_namespace("NEOPILOT_RESULT_BUF_HL")
...
---@class neopilot.Sidebar
local Sidebar = {}
Sidebar.__index = Sidebar

---@class neopilot.CodeState
---@field winid integer
---@field bufnr integer
---@field selection neopilot.SelectionResult | nil
---@field old_winhl string | nil

---@class neopilot.Sidebar
---@field id integer
---@field augroup integer
---@field code neopilot.CodeState
...
---@field chat_history neopilot.ChatHistory | nil
---@field current_state neopilot.GenerateState | nil
...
---@field old_result_lines neopilot.ui.Line[]
...
function Sidebar:reset()
  -- Cancel any in-flight LLM requests
  Llm.cancel_inflight_request()

  -- Clean up timers and debounced functions
  if self._debounced_show_input_hint then
    self._debounced_show_input_hint:close()
    self._debounced_show_input_hint = nil
  end

  -- Clean up state timer
  if self.state_timer then
    pcall(function()
      self.state_timer:stop()
      self.state_timer:close()
    end)
  ...
  -- Reset state
  self.code = { bufnr = 0, winid = 0, selection = nil, old_winhl = nil }
  self.scroll = true
...
---@field selection? neopilot.SelectionResult
  ...
  vim.api.nvim_create_autocmd("WinNew", {
    ...
    callback = function()
      for _, winid in ipairs(vim.api.nvim_list_wins()) do
        ...
        if
          winhl:find(Highlights.NEOPILOT_SIDEBAR_WIN_SEPARATOR)
...
function Sidebar:set_code_winhl()
  ...
  if Utils.should_hidden_border(self.code.winid, self.containers.result.winid) then
    ...
    table.insert(new_pieces, "WinSeparator:" .. Highlights.NEOPILOT_SIDEBAR_WIN_SEPARATOR)
...
function Sidebar:close(opts)
  ...
  -- Restore window focus if requested
  ...
  -- Recover window highlights
  self:recover_code_winhl()
end

function Sidebar:shutdown()
  -- Close will handle LLM request cancellation and cleanup
  self:close()
...
---@class NeopilotReplacementResult
...
---@return NeopilotReplacementResult
...
---@param replacement NeopilotReplacementResult
---@return string
...
---@return table<string, NeopilotCodeSnippet[]>
...
---@class NeopilotRespUserRequestBlock
---@field start_line number 1-indexed
---@field end_line number 1-indexed
---@field content string

---@param position? integer
---@return NeopilotRespUserRequestBlock | nil
...
---@param snippet NeopilotCodeSnippet
---@return NeopilotCodeSnippet[]
...
---@param snippets NeopilotCodeSnippet[]
---@return table<string, NeopilotCodeSnippet[]>
...
local base_win_options = {
  ...
  winhighlight = "CursorLine:Normal,CursorColumn:Normal,WinSeparator:"
    .. Highlights.NEOPILOT_SIDEBAR_WIN_SEPARATOR
    .. ",Normal:"
    .. Highlights.NEOPILOT_SIDEBAR_NORMAL,
  winbar = "",
...
function Sidebar:render_header(winid, bufnr, header_text, hl, reverse_hl)
  ...
  if Config.windows.sidebar_header.align == "left" then
    winbar_text = header_text .. "%=" .. format_segment("", Highlights.NEOPILOT_SIDEBAR_WIN_HORIZONTAL_SEPARATOR)
  elseif Config.windows.sidebar_header.align == "center" then
    winbar_text = format_segment("%=", Highlights.NEOPILOT_SIDEBAR_WIN_HORIZONTAL_SEPARATOR)
      .. header_text
      .. format_segment("%=", Highlights.NEOPILOT_SIDEBAR_WIN_HORIZONTAL_SEPARATOR)
  elseif Config.windows.sidebar_header.align == "right" then
    winbar_text = format_segment("%=", Highlights.NEOPILOT_SIDEBAR_WIN_HORIZONTAL_SEPARATOR) .. header_text
  end
...
function Sidebar:render_result()
  ...
  local header_text = Utils.icon("󰭻 ") .. "Neopilot"
  self:render_header(
  ...
  if Config.mappings.files and Config.mappings.files.add_current then
    vim.keymap.set("n", Config.mappings.files.add_current, function()
      if self:is_open() and self.file_selector:add_current_buffer() then
        vim.notify("Added current buffer to file selector", vim.log.levels.DEBUG, { title = "Neopilot" })
      else
        vim.notify("Failed to add current buffer", vim.log.levels.WARN, { title = "Neopilot" })
      end
    end, {
      desc = "neopilot: add current buffer to file selector",
      noremap = true,
      silent = true,
    })
  end
...
function Sidebar:resize()
  ...
  self:render_input()
  self:render_selected_code()
  vim.defer_fn(function() vim.cmd("NeopilotRefresh") end, 200)
end

--- Initialize the sidebar instance.
--- @return neopilot.Sidebar The Sidebar instance.
...
function Sidebar:update_content(content, opts)
  ...
  -- 批量设置选项
  api.nvim_set_option_value("filetype", "Neopilot", { buf = bufnr })
...
---@param selected_code NeopilotSelectedCode?
---@return string
...
function Sidebar:render_state()
  ...
  local hl = "NeopilotStateSpinnerGenerating"
  if self.current_state == "tool calling" then hl = "NeopilotStateSpinnerToolCalling" end
  if self.current_state == "failed" then hl = "NeopilotStateSpinnerFailed" end
  if self.current_state == "succeeded" then hl = "NeopilotStateSpinnerSucceeded" end
  if self.current_state == "searching" then hl = "NeopilotStateSpinnerSearching" end
  if self.current_state == "thinking" then hl = "NeopilotStateSpinnerThinking" end
  if self.current_state == "compacting" then hl = "NeopilotStateSpinnerCompacting" end
...
- If there is no AGENTS.md file in the project root directory, create a new AGENTS.md file and write the new content in it.]]
  ...
  vim.api.nvim_exec_autocmds("User", { pattern = "NeopilotInputSubmitted", data = { request = user_input } })
end
...
---@param messages neopilot.HistoryMessage | neopilot.HistoryMessage[]
...
---@param messages NeopilotLLMMessage | NeopilotLLMMessage[]
...
function Sidebar:add_chat_history(messages, options)
  ...
  for _, message in ipairs(messages) do
    ...
    if role == "system" and type(message.content) == "string" then
      ...
      ---@type NeopilotLLMMessageContentItem
...
function Sidebar:create_selected_code_container()
  ...
  if self.code.selection ~= nil then
    self.containers.selected_code = Split({
      ...
      buf_options = vim.tbl_deep_extend("force", buf_options, { filetype = "NeopilotSelectedCode" }),
...
function Sidebar:show_input_hint()
  ...
  api.nvim_buf_set_extmark(buf, INPUT_HINT_NAMESPACE, 0, 0, { hl_group = "NeopilotPopupHint", end_col = #hint_text })
...
function Sidebar:show_selected_files_hint()
  ...
  api.nvim_buf_set_extmark(
    ...
    col_number,
    {
      virt_text = { { hint, "NeopilotInlineHint" } },
      virt_text_pos = "right_align",
      hl_group = "NeopilotInlineHint",
      priority = PRIORITY,
    }
...
---@return neopilot.HistoryMessage[]
...
---@param request string
---@param cb? fun(opts: NeopilotGeneratePromptsOptions): nil
function Sidebar:get_generate_prompts_options(request, cb)
  ...
  table.insert(tools, {
    ...
    ---@type NeopilotLLMToolFunc<{ rel_path: string }>
    func = function(input)
  ...
  ---@type NeopilotGeneratePromptsOptions
  local prompts_opts = {
    ask = ask,
...
function Sidebar:create_input_container()
  ...
  local function handle_submit(request)
    ...
    ---@param log string
    ---@param state NeopilotLLMToolUseState
    ...
    self:get_generate_prompts_options(request, function(generate_prompts_options)
      ...
      local stream_options = vim.tbl_deep_extend("force", generate_prompts_options, {
        ...
        session_ctx = {},
        ---@param usage neopilot.LLMTokenUsage
      ...
      ---@param pending_compaction_history_messages neopilot.HistoryMessage[]
  ...
  local function on_submit()
    if not vim.g.neopilot_login then
      Utils.warn("Sending message to fast!, API key is not yet set", { title = "Neopilot" })
      return
    end
    if not Utils.is_valid_container(self.containers.input) then return end
  ...
  local function place_sign_at_first_line(bufnr)
    local group = "neopilot_input_prompt_group"

    fn.sign_unplace(group, { buffer = bufnr })

    fn.sign_place(0, group, "NeopilotInputPromptSign", bufnr, { lnum = 1 })
  end
  ...
  api.nvim_set_option_value("filetype", "NeopilotInput", { buf = self.containers.input.bufnr })
  ...
  api.nvim_create_autocmd("User", {
    group = self.augroup,
    pattern = "NeopilotInputSubmitted",
    callback = function(ev)
...
function Sidebar:render(opts)
  ...
  self.augroup =
    api.nvim_create_augroup("neopilot_sidebar_" .. self.id .. self.containers.result.winid, { clear = true })
...
function Sidebar:create_selected_files_container()
  ...
  self.containers.selected_files = Split({
    ...
    buf_options = vim.tbl_deep_extend("force", buf_options, {
      ...
      buftype = "nofile",
      bufhidden = "wipe",
      filetype = "NeopilotSelectedFiles",
    }),
    win_options = vim.tbl_deep_extend("force", base_win_options, {
      fillchars = Config.windows.fillchars,
    }),
