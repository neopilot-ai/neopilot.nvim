...
local Provider = require("neopilot.providers")
...
local NAMESPACE = api.nvim_create_namespace("neopilot_selection")
local SELECTED_CODE_NAMESPACE = api.nvim_create_namespace("neopilot_selected_code")
local PRIORITY = (vim.hl or vim.highlight).priorities.user

---@class neopilot.Selection
---@field id integer
...
---@field prompt_input neopilot.ui.PromptInput | nil
local Selection = {}
...
function Selection:new(id)
  return setmetatable({
    ...
    augroup = api.nvim_create_augroup("neopilot_selection_" .. id, { clear = true }),
    selection = nil,
    cursor_pos = nil,
    code_winid = nil,
    code_bufnr = nil,
...
function Selection:show_shortcuts_hints_popup()
  ...
  self.shortcuts_extmark_id = api.nvim_buf_set_extmark(0, NAMESPACE, virt_text_line, -1, {
    virt_text = { { hint_text, "NeopilotInlineHint" } },
    virt_text_pos = "eol",
    priority = PRIORITY,
  })
end
...
function Selection:submit_input(input)
  if not input then
    Utils.error("No input provided", { once = true, title = "Neopilot" })
    return
  end
  ...
  local function on_stop(stop_opts)
    if stop_opts.error then
      -- NOTE: in Ubuntu 22.04+ you will see this ignorable error from ~/.local/share/nvim/lazy/neopilot.nvim/lua/neopilot/llm.lua `on_error = function(err)`, check to avoid showing this error.
...
function Selection:create_editing_input(request, line1, line2)
  ...
  if self.selection == nil then
    Utils.error("No visual selection found", { once = true, title = "Neopilot" })
    return
  end
  ...
  local prompt_input = PromptInput:new({
    ...
    win_opts = {
      ...
      title = { { "Neopilot edit selected block", "FloatTitle" } },
    },
...
function Selection:on_entering_visual_mode(bufnr)
  ...
  self.visual_mode_augroup = api.nvim_create_augroup("neopilot_selection_visual_" .. self.id, { clear = true })
