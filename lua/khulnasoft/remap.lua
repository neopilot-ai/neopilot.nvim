...
pcall(function()
  vim.keymap.set("n", "<leader>tf", "<Plug>PlenaryTestFile", { noremap = false, silent = false, desc = "Test current file (PlenaryTestFile)" })
end)
...
pcall(function()
  vim.keymap.set("n", "<leader>f", function() require("conform").format({ bufnr = 0 }) end, { desc = "Format buffer (conform.nvim)" })
end)
...
pcall(function()
  vim.keymap.set("n", "<leader>ca", function() require("cellular-automaton").start_animation("make_it_rain") end, { desc = "Start cellular automaton (make it rain)" })
end)
